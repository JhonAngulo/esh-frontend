...
const Door_SensorDevice = ({ device, variables, sendEvent}) => {

  const [tripped, setTripped] = useState(variables.filter((item) => item.name === 'dw_state')[0].value === 'dw_is_opened' ? true : false)
  ...
  useEffect(() => {
    ...
    setTripped(variables.filter((item) => item.name === 'dw_state')[0].value === 'dw_is_opened' ? true : false)
...
export default Door_SensorDevice